export const streetRouteHeadCells = [
    { id: 'streetName', numeric: false, disablePadding: true, label: 'Street' },
    { id: 'district', numeric: false, disablePadding: true, label: 'District' },
    { id: 'city', numeric: false, disablePadding: true, label: 'City' },
    { id: 'numTrashArea', numeric: true, disablePadding: false, label: 'Trash Area number' },
    { id: 'info', numeric: false, disablePadding: false, label: 'Info' },
];

export const trashAreaHeadCells = [
    { id: 'numReq', numeric: false, disablePadding: true, label: 'Number of Request' },
    { id: 'size', numeric: false, disablePadding: true, label: 'Size(m)' },
    { id: 'weight', numeric: false, disablePadding: true, label: 'Weight(kg)' },
    { id: 'type', numeric: false, disablePadding: true, label: 'Type' }
]

export const driverHeadCells = [
    { id: 'name', nummeric: false, disablePadding: true, label: 'Name' },

]